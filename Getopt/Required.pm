package Getopt::Required;

# Name:
#   Getopt::Required.
#
# Documentation:
#   This documentation is out of date.  I have added two functions to the Module.
...
#   Chuck McKenna <cmckenna@christianacare.org>
#
# Licence:
...
#
# Additional:
#   'verAllOpt' and 'isRequired' are Freely Available and have no 
...
sub isRequired
{
    ...
    my $self = @vars[0];
    my $opt  = @vars[1];
    my $item = ${$self -> {'switch'} }{$opt};
    my $req = ${$self -> {'default'} }{$opt}{'required'};
    my $verb = ${$self -> {'default'} }{$opt}{'verbose'};
    if (( length($item) == 0 ) && ( $req == 1 )){
        return $verb;
    } else {
        return 0;
    }
}   # End of isRequired -- returns 1 if the var is ok 0 if it is not.
...
sub verAllOpt
{
    ...
    {   
        my $varLen = length(${$self->{'switch'} }{$_});
        if ( (${$self->{'default'} }{$_}{'required'} == 1 ) && ( $varLen == 0 ) ) {
            return " $_", ' ' x ($fieldWidth - (1 + length) ),"Required field missing\n";
        } 
    }
    return 0;
}   # End of verAllOpt
...
C<Getopt::Required> - Provide a Required wrapper around Getopt::Long.
...
#       'verbose'   => '',  # Not needed on every key.
        'required'  => '0', # is this option required (1) or not (0)
        'order'     => 1,
        },
    'username' =>
        {
    ...
    if (! $option -> getOptions($options, "Usage: testRequired.pl [options]") )
    {
        exit(-1);   # Failure.
    }
    ...
    # Or, after 'use Getopt::Required qw($switch);' ...
    # print "username: $switch->{'username'}. \n";
    # print "password: $switch->{'password'}. \n";

=head1 DESCRIPTION

The C<Getopt::Required> module provides a Required way of specifying:
...
You install C<Getopt::Required>, as you would install any perl module library,
...
If you want to install a private copy of C<Getopt::Required> in your home
...
C<Getopt::Required> was written by Ron Savage I<E<lt>ron@savage.net.auE<gt>> in 1997.
